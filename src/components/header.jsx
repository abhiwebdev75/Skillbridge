    ...
    <header className="fixed top-0 left-0 w-full bg-white bg-opacity-90 backdrop-blur-md z-50 shadow-sm">
      <div className="container mx-auto px-4 py-4 flex justify-between items-center">
        ...
        <nav className="hidden md:flex space-x-6">
          ...
          <Link to="/about" className="text-gray-600 hover:text-blue-500 transition-colors duration-300">About</Link>
        </nav>
      ...
      {isMenuOpen && (
        <nav className="md:hidden bg-white bg-opacity-90 backdrop-blur-md shadow-inner py-4">
          <div className="flex flex-col items-center space-y-4">
            ...
            <Link to="/about" className="text-gray-600 hover:text-blue-500 transition-colors duration-300" onClick={() => setIsMenuOpen(false)}>About</Link>
          </div>
        </nav>
      )}
    </header>
